...
- **嵌入端**（`generator.py:132-137`）：只对 simple block 做常规验证和重试嵌入。compound block 仅作为可选的被动 fallback，不计入 `total_blocks`。
...
def detect(self, code: str) -> DetectionResult:
    ...
    # score_all 接受 simple_blocks + all_blocks（用于 parent 查找）
    scores = self._scorer.score_all(simple_blocks, blocks)

    # Simple blocks 天然独立，跳过 DP selector
...
无需代码修改。pipeline 调用 `detector.detect()` ——内部逻辑已变更。报告中 `independent_blocks` 的语义从"DP 选出的块数"变为"全部 simple block 数"，更准确。

### 保留不变

- `dp_selector.py`：代码保留，不删除。当将来启用 compound block 嵌入时可重新接入。
- `hypothesis.py`：不变。
