...
`main` 的代码不是跑出这批实验的代码。四个 Code State 与 `main` 是兄弟关系而非祖先关系 —— 它们共享同一个祖先，Gated Lineage 的基点领先该祖先一个提交、`main` 领先两个 —— 且每个 Code State 还各自带着大量未提交差异。在 `main` 上直接重构，等于在一个从未跑过这些实验的版本上动手。

因此顺序是硬约束：**取物 → 逐部分分析如何合并 → 合并 → 再重构**。反过来做会把每一处服务器改动变成 delete/modify 冲突，而且是在看不见原意的情况下盲解。最直接的例子：本 spec 要求移除门控验证模块，而 Gated Lineage 工作区里那个文件是被修改过的；先删再合，改动意图就丢了。

合并面的规模：约二十五个包模块分别在四个 Code State 里存在差异，另有三个只存在于其中一个 Code State 的模块目录、一个只存在于该处的多语言实验入口脚本、一批散落的补丁文件、若干实验配置与编排脚本，以及本地工作区独有的一处生成器增量。逐部分分析意味着对每个模块回答：四个版本里哪些差异是同一意图的不同阶段、哪些是互斥的方案分支、哪些只是陈旧，以及合并后的单一版本取什么。

合并阶段不引入本 spec 的任何重构。重构在合并完成、且合并结果能跑通之后开始。

### 执行阶段

施工分四个阶段，顺序不可调换，理由见上一节。每个阶段有明确的进入门槛，门槛不满足就停下报告，不得用替代物顶上。

**阶段 0 — 取物。** 服务器端全程只读、零写入，且不产生任何仓库内改动。五项：

1. 续跑易失盘抢救。目标机器的临时目录挂在容器 overlay 根盘上而非持久数据卷，重启即失。其中有 Python Reproduction Target 的驱动脚本、配置、输入行与 pass 补全脚本，另有约一百个分析脚本、约一百个补丁文件，以及三个同族对照实验变体。排除其中一个约三百 MB 的无关二进制后，需抢救约一百一十 MB。上一次尝试被中断，仅落地七十九个文件约一点二 MB，未覆盖目标目录本身；续跑须从头覆盖而不是增量续接。
2. 拉四个 Code State 的代码面：包代码、脚本、配置、测试、工具与根级说明文件。排除 git 目录、数据目录、字节码缓存、测试缓存与门控缓存。
3. 拉只存在于 run 目录里的评测脚本、七个 Reproduction Target 的启动脚本与各 manifest。启动脚本是每次运行参数的唯一记录，属配置而非产物。
4. 取三处密钥位置的内容到仓库之外的本地目录并限制权限。
5. 抓取 Gated Lineage 的基点提交；顺带保险抓取一个只存在于服务器上的 Semantic-V3 提交 —— 它属后续轮次，但零成本，不做就是永久丢失。

落地位置：抢救物、代码面与 run 侧脚本落到仓库外的暂存目录，代码面按 Code State 分子目录；密钥落到仓库外的独立目录；提交落进本地 git。

**进入阶段 1 的门槛**：四个 Code State 的代码面齐全，易失盘抢救覆盖到 Python Reproduction Target 的全部依赖，基点提交在本地可解析。任一不满足则停下报告。

**阶段 1 — 逐部分分析如何合并。** 只产出结论，不改代码。约二十五个包模块在四个 Code State 里存在差异，另有三个只存在于其中一个 Code State 的模块目录、一个只在该处的多语言实验入口脚本、一批散落补丁、若干实验配置与编排脚本，以及本地工作区独有的一处生成器增量。对每个模块回答并记录：四个版本里哪些差异是同一意图的不同阶段、哪些是互斥的方案分支、哪些只是陈旧、合并后的单一版本取什么。遇到无法判断取舍的差异，停下询问，不得自行择一。

**进入阶段 2 的门槛**：每个存在差异的模块都有书面结论。

**阶段 2 — 合并落地并跑通。** 按阶段 1 的结论逐模块小步提交，一个模块一个提交，不做大合并提交。此阶段不引入本 spec 的任何重构，包括不得顺手删除后续要移除的模块。

**进入阶段 3 的门槛**：合并结果能跑通至少一个目标数据集的完整八阶段链。跑不通就留在阶段 2。

**阶段 3 — 重构。** 四条测试缝按测试先行推进；另需把 `encoder` 从可选阶段提升为必经阶段并按数据集拆分。四条缝与阶段序列改动详见后续各节。

**完成门槛**：四条缝的测试全绿，全量测试套件跑过一次，代码经过评审，且 Metric Contract 能对七个 Reproduction Target 各产出一行。
