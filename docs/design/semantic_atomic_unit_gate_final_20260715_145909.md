...
**文档版本：** 0.2-round-1-repair  
**语法目标：** Python 3.12  
**研究状态：** 第一轮 nightmare 审查后协议修订；没有运行本方案训练、Oracle pilot 或端到端实验  
...
第一轮审查揭示了三类不能靠“更多训练”修复的协议缺陷，0.2 已作结构性收缩：
...
4. 完整detector-side Gate/erasure/anchor/query-family selection在独立、分层null corpus上同时控制per-query、worst-stratum与atomic-family FPR，并保有detection power；embedding-side Pre plan另做causal audit但不成为blind evidence。
...
嵌入端在生成当前单元 \(U_t\) 时，可读取 \(U_{t-3:t-1}\) 作为 feasibility context，但正式载荷只由当前单元候选 \(x_t\) 的 public region \(R_P(x_t)\) 与 candidate-independent anchor \(a_t\) 的 keyed target 决定。这里必须区分两件事：第8节causal structural 4-gram是**同步地址**，只含candidate-invariant skeleton/ownership并用于HMAC寻址；它不是semantic encoder输入。observed region/bit、candidate属于哪一side及RegionV1 rejection完全只读当前unit。若把前序canonical text/embedding送入RegionV1，或让target-selected前序改写改变当前side，均是协议失败。提取端从 \(D,P,M,K\) 重新得到每个单元的 `match / mismatch / erasure`，再做相关性修正和经验校准检测。
...
- 超出bounded k-gram survival scope的无界 insertion/deletion/reorder。
- 对公开registered marker做全局canonical folding后仍保持可检测；v1明确不作此保证。
- 执行环境通过`sys.settrace`、coverage/position events、traceback对象/格式化的行列信息、读取source bytes、`inspect`或code-object/line-table introspection来区分两个catalog realizations；这些观察被`OperationalDigestV1`明确排除。

对注册 edit channel，目标是未受影响单元的 anchor 保持、受影响局部成为新 null-like evidence 或 erasure，而不是声称恢复不存在的原始状态。超出 admission/query contract 时返回 `UNAVAILABLE` 或不提供 family-wise 保证；不能写成“这些攻击只会 erasure”。若 attack 后仍 positive，必须由 held-out attack evaluation 证明，而非由设计宣称。
...
0.1 的 ordinal/virtual-slot DP 被删除：extractor没有原序列，不能比较“expected role at deleted slot”，更不能用keyed match score决定结构身份。0.2改用final-code自包含的local anchor。
...
- excluded/noncarrier simple：保留完整node/role/operator topology，identifier改为scope-local alpha role，literal只保留type/shape bucket；
...
该anchor允许不同documents的同型local structures碰撞；v1明确不提供document-unique provenance。跨文档完整复制/同构transplant是known limitation。删除`document_nonce`后，detector API不得接受nonce字段；同一canonical code重复检测得到相同anchors/targets/statistic，nonce sweep gold test必须全部拒绝未知字段。
...
extractor没有“旧anchor changed”标签，因此局部变化后的unique unit可能形成新的match/mismatch；正文不把它虚称为erasure。鲁棒性来自未受影响anchors保持，而不是密钥选择的alignment max。primary detector没有alignment hypotheses、edit penalty或DP cap。
...
3. wrong recovery率——0.2算法不声称恢复旧slot，故任何changed anchor被报告为not recovered；
4. detection survival与完整edited-null FPR。

### 8.4 Formatting、rename、reorder与duplicate

- formatting不改变AST/CST skeleton，预期anchor不变，仍须gold验证；
- alpha-rename因identifier role masking通常不改变skeleton；scope/binding结构改变则局部/subtree改变；
...
|---|---|---|---|
| `INT_ID_V1` | exact `int`，不含bool | `v`, `+v`, `v+0`, `v-0`, `v|0`, `v^0`, `v*1`, `v//1`, `~~v` | exact evaluator无overflow/exception；结果type/value与v相同 |
...
再把baseline与单一替换后的**完整module**用manifest固定的CPython 3.12 patch执行`compile(source,"<sage-c>","exec",flags=0,dont_inherit=True,optimize=0)`，比较`OperationalDigestV1`。digest preimage是deterministic CBOR递归code-object fixed array：`[co_argcount,co_posonlyargcount,co_kwonlyargcount,co_nlocals,co_stacksize,co_flags,co_code,typed_co_consts,co_names,co_varnames,co_cellvars,co_freevars,co_exceptiontable,co_name,co_qualname]`；整数均为registered unsigned widths，字符串为strict UTF-8。typed constants支持`None,bool,int,float(raw IEEE bits，仅用于周围非carrier code),complex(two raw bits),str UTF-8,bytes,tuple,immutable frozenset(sorted by encoded bytes),Ellipsis,code-object-recursion`；每项有type tag，unknown constant type或cycle fail closed。唯一有意忽略的是`co_filename,co_firstlineno,co_linetable`及其派生position tables，因为threat model排除source/position观察；CPython 3.12若出现任何其他code-object field，release checker必须显式列为included或documented-nonsemantic并触发新protocol，不能默认忽略。比较完整CBOR bytes并另存SHA-256。fresh compile process禁止quickening profile复用。任何included字段差异拒绝。
...
若未来研究header，必须先提交不读取target-selected header的两侧body branch commitment、完整成本上界、body/header pair verifier以及“retry only current header”的证明，并创建新protocol与fresh review。上下文仍不得进入header region。0.2不提供这个算法，因此对v1的结论是明确`NO HEADER CARRIER`，没有待定promotion threshold。
...
HumanEval 与 MBPP 以及其已知衍生/镜像只进入封存的 external evaluation；不训练Gate、不设计RegionV1/rewrite catalog，不做threshold/ECC/parser policy调整。[HumanEval](https://arxiv.org/abs/2107.03374) 与 [MBPP](https://arxiv.org/abs/2108.07732) 的任务文本和 canonical solutions 均进入 contamination signatures。若训练 source 与其 MinHash/AST clone 命中，整个 connected component 从训练删除。
...
- `Post-Gate-X`：只输入final current unit（前三slots BOS）；soft labels仅为current-observable/intrinsic的quality rubric与format/registered-alpha-rename robustness；reference replay由artifact conformance给hard bit。syntax/structure、duplicate/correlation由规则层计算，**不含pairwise semantic head或`p_formal(relative baseline)`**。
...
X labels有唯一recipe：`quality_target=1`要求public lint/complexity hard rules全过，且3名不知道region/target/template ID的rater按发布的1–5 readability rubric之median≥4、无任一“semantic suspicion”flag；rater不足3或Krippendorff alpha<0.67的batch为unknown而非positive。`format_target=1`要求固定formatter artifact运行后current `RegionCurrentV2`、BlindCertificate与GateProjection byte-exact；`rename_target=1`要求预注册的8个capture-avoiding local alpha-renames全部保持这些字段。`intrinsic_accept_target=quality∧format∧rename`，仅三项全known时定义。parser、BlindCertificate、backend replay和duplicate/cluster不是soft labels，分别是hard bits。人工/编辑真值只作监督，绝不进入tensor。
...
2. Pre输出经独立校准的`LCB(q0),LCB(q1),LCB(qboth),cost`；其中`cost_q12=max(cost0_q12,cost1_q12)`是第23.1节的fixed-point first-success fraction，`safe_q12=min(LCB0_q12,LCB1_q12,LCBboth_q12)`。未满足第21节\(G_i^{pre}\)的unit令\(u_i=-1\)并不分配；其余整数utility `u_i=floor(2^20·safe_q12/max(1,cost_q12))`；所有LCB/cost先由注册LUT映射为Q12整数；
3. 先按`(-u_i,anchor_hash)`得到唯一静态顺序，再逐项根据该unit code-object scope的当前\(m\)计算marginal \(c_{i|m}\)；若\(m=4\)或加入会超过`B_doc`则跳过继续，否则扣减budget并增加scope count。不会因marginal变化重排已冻结顺序。这是public partition-cap greedy，不按pool outcome回溯。no-Gate baseline按`(source_order,anchor_hash)`、rule-Gate按`(-rule_score,anchor_hash)`冻结顺序，二者都使用相同scope cap与marginal reservation；rule-Gate若为所有eligible units预枚举raw template/region sides，必须先按相同token/hash RGE逐项扣除该cheap census成本，预算不足的units取最低rule score，不能把预计算当免费oracle；
...
target解锁前运行`PLAN_SIGNAL_GUARD`：composition-certified dual slots≥16、按第29节只看这些slots形成的public clusters≥8、exact \(n_{eff}\ge12\)。其结果随后写入commitment。失败时不调用任何key API，只能返回明确标记为**非水印**且不含protocol metadata的\(D_0\)，或`NO_FORMAL_ARTIFACT`；该document计入candidate/system coverage denominator。guard通过也不保证final detector一定positive，只是排除明显无信号plan。guard、阈值与结果进private audit/commitment，不进提取输入。
...
两侧都非空且两个winners均预提交，所以不存在target-conditioned count/retry/fallback。最终`BLIND_REPLAY(D1)`与`PAIR_VERIFY_DETERMINISTIC(D0,D1)`是defense-in-depth assertion，不能把blind replay写成功能验证；后者只重放exact evaluator、span/skeleton、OperationalDigest与ScopeCompositionCertificate，不在target后重新运行可能flaky的project tests。E2/E3动态结果只在target前运行并commit为候选tier/诊断；held-out functionality只用于system evaluation，不控制某个target后的发布。在符合规范的实现里final assertions应由上述引理保证。若仍出现UNKNOWN/FAIL，不发布artifact、不回退D0、不以相同或新plan重试，并立即停用该protocol/key作为`POST_TARGET_SECURITY_INCIDENT`；任何一次该事件都使Layer-3 Hard criterion失败，因为release availability已可能成为一位侧信道。

### 16.3 泄漏审计

在同一frozen \(D_0\)下对1,000个keys重放planning，要求document commitment、Pre/allocator、candidate/sentinel bytes/order/count/config、scores、dual/composition status、two winners、scope certificates与timeouts byte-identical。plan controller先验证完整commitment再一次性解锁target process；target interface有deny-by-default schema，只接收`(K_target,protocol_id,anchor_hash,block_info)`，不能调用generator、Gate、pool builder或compiler。selector使用的winner pair必须来自解锁时已冻结的只读plan mapping，运行时hash再次对document commitment做全量校验。
...
| `anchor_preimage:bytes` | 是 | conformance | PRF input | PRF input | 不含current text/key | anchor-v2 |
| `anchor_hash:sha256` | 是 | join only | PRF input | PRF input | 与per-unit response联合可成oracle；不发布response | hash suite |
...
**不存在**`document_nonce`、virtual slot、sync ordinal或caller-supplied anchor字段；unknown input field fail closed。
...
| `reservation_RGE:u32,allocator_state:enum` | 条件 | cost label | target前 | 否 | budget trace；禁tensor | allocator-v1 |
...
| `exact_eval_digest,operational_before,operational_after:sha256` | 条件 | pair labels | hard filter | unavailable baseline | privileged pair info；禁Post-X | certificate-v1 |
...
| `min_rank_0,min_rank_1:u16/null,cost_RGE:float32` | 条件 | cost targets | 否 | 否 | attempt trace；禁input | cost-v1 |
...
| `quality_target,format_target,rename_target:bool/float/unknown` | 条件 | X supervision with masks | X predict | X predict | no key; label missingness mask | rubric/edit release |
...
| `intrinsic_accept_target:bool` | 条件 | X asymmetric target | threshold select | threshold apply | only conjunction of final-observable labels | policy-v2 |
...
| `assembly_kind:enum(natural,baseline_fallback,multi_replacement,prompt_like,format,rename,edit,random_side_null,wrong_key_null)` | aggregate | sampler stratum only | key-free replay | unavailable | trajectory shortcut；禁tensor | final-replay-v1 |
...
| `q0,q1,qboth,cost,utility:int32` | 否 | diagnostics | allocator | not used | selection shortcut；不进final | gate/calibration |
...
| `cluster_id,block_anchor:sha256,block_position:u8,cluster_weight_q30:i64` | aggregate | 否 | 否 | derive | identifiers default private | cluster/ECC-v2 |
...
| `dependency_hashes:map[enum(canonicalizer,region_projection,gate_projection,parser,skeleton,anchor,feature_schema,rewrite_catalog,blind_certificate,composition,region_spec,resource_contract),sha256]` | 是 | bind | verify | verify | dependency integrity | protocol-v2 |
| `thresholds_int:map[head,struct{reject:i16,accept:i16}],calibration_LUT:map[head,fixed[u16;65536]]` | 是 | certification output | use | use | public decision | calibration release |
| `rejection_bands:map[head,struct{low:i16,high:i16}]` | 是 | certification output | use | use | boundary→erasure | calibration release |
...
每个public stratum的null releases必须分别含四个construction groups且各自满足第31节样本量：(1)自然未加水印代码；(2)第14.4节key-neutral random-side marker assemblies，用于匹配marker/evidence count；(3)在独立组件上生成的watermarked assembly用independent wrong key检测；(4)format/edit/error-channel null。group 2/3不能与positive test共享prompt/task/component。所有null与positive来自同支持长度/结构/evidence strata；同一prompt/task的variants属于同一connected component，不能跨任何calibration/test partition。repository/task/generator/domain虽然不作为query路由输入，仍作为hidden audit groups，在risk-cert/test上做simultaneous worst-group bounds。
...
1. `certificate capacity`：只计strict parse、skeleton invariant、exact evaluator与OperationalDigestV1全过；
...
对每个anchor、public pool variant \(r\) 和region \(b\)只在non-identity records上计算`Y0r,Y1r,Yboth,r=Y0rY1r`、有限variant精确比例、最小rank、side correlation、canonical class size、certificate/dynamic survival及format/rename/edit replay；identity C0另报但永不算side success。跨pool“ever reached side0 and side1”只作诊断，绝不称deployment dual-complete。formal四variants不报告伪随机Jeffreys posterior；不确定性来自按rewrite family/value type/generator/repository聚类的anchor/repository bootstrap与actual-deployment-variant binomial CI。只有另行抽样的oracle-only generator pools才可同时报Jeffreys posterior。所有结果报micro/macro；\(A=2\) dual rate应精确为0，是accounting gold check。
...
formal evidence/1000 generated tokens、attempt-success curve、marginal gain、zero-entropy share、format/rename/local-edit RegionV1+anchor replay。headers只报noncarrier diagnostic，不进入容量分母或promotion。
...
ECE只作固定15个equal-mass bins的评估指标；不把未定义SoftECE写进loss。posthoc monotone integer isotonic LUT只在calibration-map split拟合。
...
Post calibration-map 5%先拟合Post LUT；Post threshold-selection 2.5%枚举Post-X integer\((T_R,T_A,\{\theta_m^R,\theta_m^A\}_{m\in\mathcal M_X})\)，只用FinalReplayCalibrationRecord而非candidate rows，覆盖natural/key-neutral fallback/multi-D1/edit/null strata中的**registered non-identity replay forms**；identity rows只验证rule erasure，不进入Post FAR/coverage分母。在满足每对reject<accept、经验FAR≤0.2%的组合中选coverage最高者，lexicographic integer tuple破tie，然后永久冻结Post backbone/heads/LUT/thresholds。joint risk-cert按document/anchor family计算one-sided CP与component-bootstrap UCB并取max；正式要求overall及每个registered final-stream stratum UCB≤0.5%。confidence alpha按heads×strata×hidden audit groups同时分配，记cell allocation为\(\delta_{cell}\)。每cell negatives至少为\(\max[1000,\lceil\log(\delta_{cell})/\log(0.995)\rceil]\)，这是零false accept时让CP upper≤0.5%的最低量；出现failure时继续按精确CP反算更大n，不能固定1000后宣称达标。另要求≥20 repositories、单repo≤10%；不足即unsupported。unknown只报coverage。
...
没有document nonce。target只由protocol-bound local anchor与domain-separated keys决定；同一code/protocol/key重复检测是同一个test。从master key经HKDF-SHA256派生互异`K_msg,K_mask,K_unit`。非ECC单元target：

\[
b_t=LSB\,HMAC_{K_{unit}}(\texttt{"SAGE-C/unit/v2"}\|anchor_t).
\]

第30节定义的每个content-landmark ECC block有共同、明确的`block_anchor`与4-bit check message：

\[
m_s=\operatorname{Trunc}_4 HMAC_{K_{msg}}(\texttt{"SAGE-C/ecc-msg/v2"}\|block\_anchor_s),
\]

经第 30 节 Hamming(7,4) 得 codeword \(c_{s,0:6}\)。slot \(j\) 的正式 target：

\[
b_t=c_{s,j}\oplus\operatorname{LSB}HMAC_{K_{mask}}(\texttt{"SAGE-C/ecc-mask/v2"}\|anchor_t).
\]

上述每个`||`在wire format中都编码为`LP(field)=u32be(len(field))||field`后再拼接；`LSB`和`Trunc4`按第30节的exact bit order。未落入完整ECC slot的unit使用`K_unit`公式。anchor不含current candidate text。ECC只是target组织/secondary diagnostic，不承载外部payload。
...
PLAN_AND_EMBED(prompt, public_protocol P, sealed_key_handle K):
  ...
  for scope in LEXICAL_CODE_OBJECT_SCOPES(T0) in structural order:
    ...
    if not cert.all_recursive_digests_equal_baseline:
  ...
  commitment = HASH_ORDERED_FULL_PLAN(P,plan,P.B_doc,guard)
  ...
  target_api = UNLOCK_TARGET_ONLY_INTERFACE(K,commitment)

  replacements = []
  for entry in plan:
    if entry.state != DUAL_COMPLETE: continue      # target never derived
    b = TARGET(target_api,entry.anchor,CONTENT_BLOCK_INFO(T0,entry))
...
生成结束后对完整final code重新strict parse/linearize/anchors/Post-X/RegionV1；任何generation“success”不升级formal evidence。artifact默认只有code。self-describing模式在shebang/UTF-8 cookie后只允许一行`# sage-c-protocol: sha256=<64 lowercase hex>`；否则API唯一指定protocol ID。重复/malformed/conflict/unknown metadata fail closed。**不存在nonce metadata/API字段**。不写per-unit state、plan、pool、target、retry或key。
...
提取器唯一输入是`(final_code bytes, protocol_id, public registry/artifacts, K)`；protocol ID可由唯一leading comment恢复，不能有nonce/subset/anchor参数：

```text
EXTRACT(final_code D, protocol_id, public registry, secret K):
  REQUIRE_EXACT_PROTOCOL_EVALUATOR_AND_CONFORMANCE()
  if request has nonce/subset/anchor/unknown field: return UNAVAILABLE_REQUEST
  ...
  for t in T.all_syntactic_units:
    ...
    blind_cert = BLIND_CERTIFICATE_V1(D,t)          # inverse template + digest
    ...
    target = TARGET(K,P,t.anchor,CONTENT_BLOCK_INFO(T,t))
  ...
  ecc = ERASURE_AWARE_ECC_REPORT(evidence,K)
  return RESULT_FOR_FAMILY_CORRECTION(p,ecc,counts,n_eff,stratum)
```

`STATIC_DOCUMENT_ADMISSION`只检查protocol/artifact/parser/encoding、canonical token length和与sign无关的静态resource limits；它不读取尚未计算的Post-X/evidence count。`DetectorResourceContractV1`进一步要求registered non-identity replay-shaped units≤512；先编译final D一次并缓存digest，每个shape至多再编译一个identity-reconstruction，故最多513次fresh compile和512次Post-X/Region calls，单compile CPU quota 1 second、document总CPU quota 600 seconds、RSS 2 GiB。任一quota/overflow失败使整个document `UNAVAILABLE_RESOURCE`，不能只取前512个units形成有利subset。`EVIDENCE_SUFFICIENCY`在全unit hard/Post-X/Region replay及duplicate suppression之后检查16–512 non-erasure、≥8 clusters、exact \(n_{eff}\ge12\) 与ambiguity≤20%；public stratum也在此后路由。这样没有“先用accepted evidence admission、后计算accepted evidence”的循环，也不会在看match sign后选择是否检测。
...
不存在protocol registry、输入超出正式长度、未知Python feature、unsupported encoding/token、runtime mismatch、integer overflow、anchor ambiguity或Post-X/RegionV1 boundary时fail closed为unavailable/erasure。不会换相近模型或比较continuous embeddings。
...
先按第30节content-landmark规则形成secondary ECC blocks；excluded/oracle-only/noncarrier positions是erasure，不能压缩重编号。再用**公开、key-independent**规则形成dependence clusters：在同lexical code-object scope且同nearest compound-owner suite内，以全部syntactic-unit的suite-local`ast_ordinal`为坐标，对两个accepted evidence在ordinal gap≤3时连无向边，cluster取该图的transitive connected components；不同suite永不连边。相同`current_text_identity`复制品先只让raw-byte start最小者保留accepted state，其余强制erasure，再建图。raw-byte tie不可能发生，若发生parser span collision则document unavailable。cluster边界不能看match/mismatch sign。
...
cluster mean让一个复杂clause的大量相似units不能线性放大证据。\(n_{eff}\)只作报告/guard，不解释为iid Bernoulli数。HMAC targets也不使units、Gate selection或ECC slots独立；尾概率完全依赖第31节分层empirical null。

### 29.2 层级输出

每个 block 报 `match/mismatch/erasure` counts 与 `ecc_verified`；每个 scope 报 cluster statistic；一个formal v1 request只接受一个Python module，并且只产生一个预注册primary \(T_K\)。多文件project不是v1检测对象：调用方必须把每个module作为独立family member提交，并按第31.3节的Holm/Bonferroni规则校正；任何project-level pooling都需要新的major protocol、key-independent file clusters、长度normalization与独立project-null calibration，不能事后挑分数最高的函数或文件。
...
SAGE-C不承载外部payload。ECC编码key-derived 4-bit check message。block identity完全公开且candidate-invariant：在每个suite，BOS是landmark；unit \(i\) 若`SHA256("SAGE-C/landmark/v1"||d_i)[31] & 0x0f == 0`也是landmark（即big-endian digest整数的low4）。每个非landmark unit归最近preceding landmark；仅该landmark**之后**的前7个all-syntactic positions组成一个block，position为0…6，后续positions无ECC直到下一landmark。landmark unit自身不占其新block position，若可formal则走`K_unit`target。令真实unit landmark的`landmark_local_signature=ell_i`（第8.2节）；suite BOS使用`SHA256("SAGE-C/landmark-bos/v1"||scope_token)`。共同block anchor：

\[
block\_anchor=SHA256(\texttt{"SAGE-C/block/v2"}\|scope\_token\|landmark\_local\_signature).
\]

同一block七项必须逐byte得到同一`block_anchor`；独立gold vectors覆盖suite BOS、近邻landmarks、insert/delete。formal rewrite不改变\(d_i\)，所以zero-edit block identity不变；插入/删除可影响当前landmark segment但不会跨下一landmark。excluded/noncarrier positions保持erasure，不能压缩。systematic Hamming(7,4,3)：
...
`Trunc4 HMAC`固定取digest第0 byte的high nibble，\((d_1,d_2,d_3,d_4)\)按bit 7→4排列；所有`LSB HMAC`固定为digest第31 byte的bit 0。position \(j\) 的target是 \(b_j=c_j\oplus LSB\,HMAC_{K_{mask}}(LP(\texttt{"SAGE-C/ecc-mask/v2"})\|LP(anchor_j))\)，其中`LP`是unsigned big-endian u32长度前缀。提取时observed region先XOR mask，erasure保持unknown，再枚举16个合法codewords作bounded-distance decode。只有最优codeword唯一、满足

\[
2e+s<3
\]

且decoded data等于重新派生\(m_s\)时才`ecc_verified=true`；同distance tie、少于5 observed或rank不足都erasure。大量excluded使许多blocks不可解，这是required measurement。
...
0.2没有alignment max；唯一primary statistic就是第29节的exact INT64 Q30值

\[
T_K(D)\equiv T_K^{Q30}(D)=RoundDiv\!\left(2^{20}\sum_c RoundDiv(2^{30}\sum_{i\in c}e_i,n_c),\ isqrt(C2^{40})\right).
\]

仍不能用二项/正态tail，因为rule/Post-X/Region erasure、clusters、natural units与ECC相关。正式admission先按§27.1检查：UTF-8/Python/protocol、4,096–65,536 canonical tokens、registered non-identity replay-shaped units≤512、16–512 accepted evidence、≥8 clusters、\(n_{eff}\ge12\)、anchor ambiguity≤20%。`accepted evidence`在比较expected bit前计算，所以不含match sign。再构造只含public/key-independent covariates的fixed Q15 vector：log2 length/syntactic/accepted/cluster bins各1维、formal-replay-shaped fraction与ambiguity fraction各1维、suite-depth bins\([0,1,2,3,4,5+]\) histogram 6维、unit-type mix`Assign/AnnAssign/header/excluded/other` 5维，共17维；count bins和每维Q15 denominator/rounding写入manifest。
...
对每个supported stratum \(s\)和第18节null construction group \(g\in\mathcal G=\{natural,random\_side,wrong\_key,edit\_error\}\)，从independent audit-key distribution与完全分离null documents取得\((K^0_{jg},D^0_{jg})\)。完整执行parser、eligibility、local anchor/ambiguity、Post-X、RegionV1、erasure、duplicates/clusters、guards、stratum routing和ECC，形成冻结artifact \(\mathcal S^0_{P,s,g}=\{T_{K_{jg}^0}(D_{jg}^0)\}_{j=1}^{N_{s,g}}\)：
...
取maximum是保守intersection decision：真实input construction group即使不可观察，其正确group p-value也不大于返回值；不能从四组中挑最小p。每个\((s,g)\)初始\(N_{s,g}\ge10,000\)，另有每组≥10,000 held-out null-test；真实sample size按目标order statistic/CI增加。risk-cert/null-test要求每个supported stratum、每个construction/hidden group与总体single-query FPR point≤1%、one-sided simultaneous95% upper≤1.25%；任一失败关闭该stratum或整个protocol。至少128 audit keys检查dispersion；production key均匀生成，qualification不能改public threshold。

null corpus覆盖registered长度/结构/evidence strata，并在hidden repository/task/generator/domain及clean/edit/error groups做worst-group audit。threshold不使用positive test。detector-side Gate/erasure selection通过current-only Post-X完整重放，不假设独立；embedding-side Pre plan不是证据，只在第24.3节的key-free causal audit中运行且不得condition tail。没有nonce，null/API也不生成或接受nonce。
...
一个`protocol_id`唯一绑定：Python/bytes/parser/CBOR/role table、eligibility/rewrite certificate、UnitSkeleton/k-gram anchor、context width、RegionV1、Gate weights与integer evaluator/opset/rounding, fixed shapes、threshold/LUT/bands、allocator/RGE、generator/pool config、HMAC/HKDF domains、content-landmark ECC、cluster/statistic/public strata/OOD/null/query-family releases与conformance vectors。registry canonical JSON的SHA-256为ID；任一normative field改变都产生新ID。
...
public：protocol manifest、parser/tokenizer/Gate/integer evaluator、RegionV1、thresholds/LUT、stratum/OOD medoids、null tails、ECC/statistic spec和可选单行protocol metadata。private：master/subkeys、per-anchor expected bits、production-key qualification与generation audits；private audit不改public threshold。没有nonce或sidecar。缺少唯一protocol ID时unavailable；unknown API fields fail closed。
...
\[
PRK=HKDF\text{-}Extract(SHA256(LP(\texttt{"SAGE-C/hkdf-salt/v2"})\|LP(protocol\_id)),K),
\]

\[
K_x=HKDF\text{-}Expand(PRK,LP(\texttt{"SAGE-C/"}x\texttt{"/v2"})\|LP(protocol\_id),32),
\quad x\in\{msg,mask,unit\}.
\]

`LP(z)=u32be(len(z))||z`，长度按bytes。所有HMAC preimages采用同一length-prefix canonical encoding；不同protocol/key rotation不复用domain。public anchor hash不是MAC，不能反推key。master/subkeys永不序列化到public schema、model、logs或final code。

攻击者可以从每个显式non-identity marker计算observed RegionV1；对真正selected carrier，这等于该anchor的PRF target，因此协议必须把它视为chosen-message下泄露若干HMAC output bits，而不是声称target不可见。安全假设是HMAC-SHA256作为PRF在这些公开anchor/output-bit pairs下仍隐藏master/subkeys；key rotation与domain separation限制跨protocol累积。per-anchor expected-bit sidecar、selected-position map与完整HMAC bytes仍禁止发布。
...
| document commitment | ordered allocator/skips/pools/scores/config | key/target | 否；完成后才解锁key |
| target derivation | anchor/block info、key | candidate generation/control | 是 |
...
1. 对同一\(D_0\)/planning RNG改变1,000个random keys，全document Pre/allocator/Post-G/X envelopes、commitment、pool bytes/order/count/seeds/dual-status/latency bins必须byte-identical；只有targets/selected candidates/D1可变。
...
删除nonce消除了“同一code枚举metadata得到新target”的额外自由度：请求nonce一律拒绝，重复artifact完全确定。但同构documents可能共享anchors/target stream，完整copy/transplant无法归属唯一generation event；解决provenance需外部signature/registry，属于non-goal且不得计watermark evidence。
...
|---|---|---|---|
| comments/blank lines/formatting | ordinary comments非unit；canonical/anchors保持 | protocol comment删后API无ID则unavailable；否则Region/Post-X应稳定 | formatting survival |
| identifier alpha-rename | masked scope anchor与RegionCurrentV2保持；Post-X GateProjection保持 | binding topology改变时局部/subtree变；否则应exact survive | rename survival ≥99.9% Hard for registered alpha-rename |
| equivalent expression rewrite | slot anchor保持；current-only region重新算 | 可翻 region，不能保证 | rewrite survival/quality |
| unit deletion | 同suite删除边界及其后至多3个原units必变；之前与更后anchors exact | changed local evidence变null-like/erasure | untouched-anchor survival/new-anchor noise |
| unit insertion/dead code | 同上；duplicate k-gram全组erasure | 局部noise或signal erase | insertion survival/FPR |
| same-suite reorder | 各删除/插入边界及其后至多3个units变化 | 不声称恢复old slots | reorder survival |
| control-flow rewrite | role path/subtree改变 | subtree evidence erasure | attack TPR/FPR |
| header/body edit | owner local signature变可改变child scope token | child subtree new anchors/erasure；syntax坏则invalid | per-clause survival |
| copy a matching unit | text-identity duplicate cluster | 只有首个有权重 | copy amplification test |
| cross-document transplant | 同构local anchors可复用targets | partial transplant可noise/match；完整copy无法归属 | transplant/copy disclosure |
...
local self-synchronization只保证同suite单edit点之前以及编辑后第4个未改原unit起的causal windows不变；header本身/其三个前驱导致的child subtree、重复4-grams和多点overlap不保证。无界编辑可清除水印。格式稳定由parser/canonicalizer contract，rename/rewrite survival仍是required experiment。
...
| null必须重放什么？ | parser、eligibility、local anchor ambiguity、Post-X、RegionV1、rejection、duplicates、clusters、guards、stratum/OOD、ECC和query-family correction。 |
...
| code formatting/rename/edit | 不适用 | recompute local anchors；unaffected survive，changed become noise/erasure | 生存率/FPR实测 |
| input nonce/subset/unknown API field | 无 | `UNAVAILABLE_REQUEST` | 防nonce/query重采样 |
...
1. **Parser/contract qualification。** 两个独立实现验证bytes/Unicode/semicolon/inline/multiline/async/header/decorator/match/exception、Canonical/Region/Gate projections、skeleton/k-gram anchor；所有gold exact 100%，否则停止。
2. **Rewrite/Region static census。** 不训练模型；验证Rewrite/BlindCertificate、template inversion、non-identity canonical classes、scope-composition combinations与RegionV1 sides。若zero-entropy/density上界失败即NO-GO。
...
5. **Stage-J joint representation。** 五个public seeds在train上用\(V^{oracle}\)辅助目标训练shared backbone、PRE/POST-G/POST-X；current-only/simple loss、CTX-1/2/3和rule/no-Gate同时保留为baselines，不在test选结构。
6. **Hard-negative mining。** 最多三轮，只从train false accepts选样，保留原prevalence权重；每轮用model-validation评估，test不参与。G只读pair+冻结D0 context；X始终current-only，pair truth不监督X。
7. **QAT/integer curriculum。** FP32稳定后加入IntGateV1 fake quant、batch companions/padding，周期执行真实integer reference；任何conformance mismatch的seed无资格进入选择。
8. **Post map/select/freeze。** 按五-seed中位model-validation utility选一个artifact；仅用Post calibration-map/threshold-selection冻结backbone、POST-G/X LUT与thresholds，不看Pre partitions/risk/test。
9. **Stage-P Pre fitting。** 在train用冻结Post pipeline重算\(V^{dep}\)，只更新Pre head；随后只用Pre calibration-map/threshold-selection冻结Pre LUT/bins/thresholds。feature/backbone/Post不得回开。
10. **Joint risk certification。** 用joint risk-cert一次认证Pre/Post所有selected thresholds、hidden groups和alpha ledger；失败不可回调模型。另用null-admission-fit与null-tail releases冻结strata/tails。
11. **System evaluation。** 一次性运行held-out repository/task/generator、null-test、HumanEval/MBPP、edit/attack、latency/cost矩阵；按第40节裁决。

训练recipe本身是有限且可重放的。Stage-J使用FP32 master weights的AdamW（`betas=(0.9,0.999), eps=1e-8, grad_norm_clip=1.0`），每batch含32个anchor groups及每组全部CTX-1/2/3；Post任务每anchor至多按固定public sampler取8个records并用inverse sampling weight恢复目标prevalence。只枚举`lr∈{1e-4,3e-4,1e-3}`、`weight_decay∈{0,1e-4}`，以及23.6节base recipe或“一次只改变一个loss group”的乘数`{0,0.5,2}`；最多80 epochs，epoch 41开始fake-integer QAT，epoch 50后按constraint-first model-validation utility patience 10 early-stop，完全相同utility时取`(architecture,lr,weight_decay,loss-group,multiplier,epoch)`字典序最小tuple。Stage-P只更新Pre head，AdamW参数相同，`lr∈{1e-4,3e-4}`、weight decay 0、最多30 epochs、patience 5。定义`protocol_candidate_hash=SHA256(CBOR_D(all normative protocol fields except learned weights, fitted LUT/thresholds/null tails and their descendant hashes))`；五个public seeds为`low64(SHA256(CBOR_D(["train-seed/v1",protocol_candidate_hash,u8(i)])))`,`i=0…4`。有效seed少于3即失败，最终按第40.2节取validation utility中位seed，tie取seed integer较小者。任何另加optimizer、epoch、sampler或组合搜索都产生新training-recipe hash并重做calibration/risk-cert。
...
| 17 | formatting survival rate | format后anchor、RegionV1、Post-X/evidence state均保持 / pre-edit formal evidence |
| 18 | rename survival rate | 分两项：registered capture-avoiding local alpha-rename的anchor/Region/Gate/evidence byte-exact率（Hard=100%）；更一般rename后仍同expected target且match / pre-edit matches（attack metric） |
| 19 | statement insertion/deletion survival | 单unit edit前及编辑边界后第4个未改原unit起、owner不变的原evidence保持exact anchor/state / 可追踪理论不受影响原evidence |
| 20 | local resynchronization rate | edited pairs中理论unaffected anchors exact保持 / all theoretical unaffected；changed anchors不算“恢复” |
...
| 23 | 经验FPR | single-query per stratum/worst-group及Q≤32 family-wise positives，simultaneous CP 95% UCB |
...
| 27 | ECC解码成功率 | unique keyed-message verified content-landmark blocks / all blocks；另报≥5-observed条件率 |
...
| 条件 | 门槛 | 类型 |
|---|---:|---|
| Rewrite/BlindCertificate/static census | 两实现枚举、template inversion、non-identity canonical forms/digests与CompositionalityLemma property tests 100%一致；identity永不计side，zero-entropy share不使density上界<4/1000 | Hard |
...
训练run protocol固定5个public optimization seeds。只用model-validation对满足export/conformance且无NaN/overflow的seeds按\(U(\pi)\)排序，选择**中位数**seed而非test-best；若有效seeds<3则training failure。Post/Pre calibration只对该预选artifact运行。Layer-2 `TRAINING SUCCESS`要求该artifact通过表中全部Hard与Initial条件，且其model-validation utility不低于5-seed median定义；任一risk-cert、unseen-group、same-budget或integer export条件失败即`TRAINING FAILED/LEARNED GATE NO-GO`，不能通过换seed看test、扩大模型或降低threshold救回。其他四seeds只报告stability，不接触held-out test。

### 40.3 Layer 3 — System feasibility

| 条件 | 门槛 | 类型 |
|---|---:|---|
| held-out null FPR | 每public stratum、hidden worst-group、single-query及Q≤32 family-wise均point≤1%、simultaneous95% upper≤1.25% | Hard |
...
| normative replay | Canonical/anchor/RegionV1/all integer tensors/Post-X/evidence 100%，cross-process byte-exact | Hard |
...
| registered normalization conformance | formatter-only与capture-avoiding local alpha-rename的Canonical projection/anchor/Region/Gate/evidence byte-exact=100% | Hard |
| broader edit survival | format≥95%，一般rename≥70%，单insert/delete theoretical-unaffected anchor survival≥90%；changed anchors另报 | Initial |
...
4. k-gram self-synchronization不恢复旧slots；编辑窗口内形成new-anchor random evidence或erasure，owner header edit可改变child subtree。大规模/duplicate/reorder可擦除signal。
...
9. Hamming(7,4)面对大量rule erasures和每code-object最多4个selected units的composition cap可能很少有≥5 observed bits；ECC不增加底层carrier容量，v1把它保留为secondary diagnostic而非挽救primary power。
...
12. 8个conditional strata×4个null construction groups的tails/test各需要至少一万、通常更多独立`(document,key)`pairs，最低即约640,000次tail+test检测，尚不含hidden groups；不能保证任意固定key或无限adaptive查询的有限样本FPR。
13. 当前只规范Python 3.12 source，不覆盖notebook cell order、generated AST bytecode、C extensions或跨文件semantic rewrites。
14. 没有document nonce；同构local structures可能共享targets，完整copy仍不可归属唯一generation event。外部signature/registry是另一个provenance机制，不能计watermark evidence。
15. 全document commitment要求buffer baseline/pools/two-side winners，不能直接发布online stream；增加内存与首个可发布artifact延迟。尽管CompositionalityLemma应在解锁前排除正常post-target失败，任何实现incident仍可能通过“无artifact”泄漏availability，因此一次事件即停用protocol/key，而不是可恢复fallback。
16. 正式支持对象是≥4,096 canonical tokens的单Python module；常见短函数/HumanEval/MBPP预计多数insufficient。若业务目标是函数级检测，本v1在scope上NO-GO。

这些限制不能用“模型更大”关闭；第1、3–8、13–16属于scope/合同或载体限制，第2属于可能的根本容量瓶颈。
...
| WP | 产物 | 必须验证后才进入下一步 |
|---:|---|---|
| 0 | `protocol.json`、exhaustive grammar/CBOR/role tables、≥1,000 gold vectors | 两实现span/CBOR/skeleton/anchor/RegionV1 100% exact |
| 1 | parser/linearizer/k-gram anchor library与property tests | edge grammar/Unicode/CRLF/fuzz零未分类；single-edit bounded-impact |
| 2 | RewriteCertificateV1 enumerator/verifier、static census、Oracle pilot | 低成本carrier-entropy falsification；失败停止 |
| 3 | provenance/split graph、10 schemas+final-replay schema、formal Oracle(A≤9) | Layer-1 thresholds/CI与clone audit |
| 4 | shared PRE/POST-G/POST-X、allocator、baselines/ablations、integer QAT export | Layer-2、deny-by-default feature audit、final-stream FAR |
| 5 | integer reference evaluator/registry/conformance vectors | ≥10k exact tensors+boundary fuzz+fail-closed matrix |
| 6 | embedding/extraction、landmark ECC、stratified statistic/query-family null | ≥10k per supported stratum×4 null groups for tails and again for independent tests、Layer-3/attacks |
| 7 | external benchmark/data/model cards和安全release review | 三层通过后才讨论production integration |

最低成本下一项不是训练Gate，而是WP0+static census/pilot：从≥10个license-cleared、互不clone repositories抽每个ClosedValue family 300 anchors，加入≥100 parser stress snippets；运行catalog exact enumeration、OperationalDigest、RegionV1，\(A=6,9,R=2\)。它只能falsify，不能给Layer-1 GO。若parser/zero-edit exact<100%、format replay<99.9%、pilot min-side<0.20或static density upper<4/1000，立即停止并修协议/判NO-GO。
...
第一轮原始意见完整保存在`review-stage/ROUND_01_REVIEW_RAW.md`与`review-stage/ROUND_01_ADVERSARIAL_RAW.md`；逐项append-only ledger在`review-stage/ISSUE_RESOLUTION_MATRIX.md`。continuing reviewer提出3 critical/11 major/4 minor，对抗reviewer提出2 critical/10 major/3 minor；作者全部`ACCEPT`。0.2正文修复包括：删除nonce与hidden-reference DP；actual same-pool dual gate；finite rewrite certificates；RegionV1；global allocator；Post-G/X分离；final-stream calibration；independent risk certification；integer evaluator；headers永久noncarrier；stratified/family FPR；short-module scope。

本轮作者修改不是reviewer closure。所有critical/major的`reviewer_closure`仍为`OPEN`，直到同一continuing reviewer查看本revision并在原始复审中逐项写`CLOSED`。尚需的实验在matrix逐项保留，没有把design修复写成实验结果。
...
| 0.2 round-1 repair | awaiting continuing re-review | N/A | NOT READY（未复审） | N/A | N/A |

## 45. Final reviewer verdict

`REVIEW PASSED: NO`（0.2提交复审前状态）。第一轮未通过；尚未完成至少两轮formal review、连续READY与fresh reviewer验收，因此不得标记Goal complete。
...
未运行实验，所以不能声称：catalog有足够双侧容量、Gate提高evidence/cost、Post-X达标、integer backend已实现/稳定、编辑鲁棒、FPR/TPR/功能/成本达标。可以声称的只有：0.2给出可实现/可证伪的final-code-only protocol candidate及明确stop conditions；经验前提仍待验证。
