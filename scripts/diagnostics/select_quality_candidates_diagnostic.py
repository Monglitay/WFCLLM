#!/usr/bin/env python
"""Run diagnostic quality-proxy candidate selection."""
...
from wfcllm.diagnostics.quality_selector import (  # noqa: E402
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Select among diagnostic candidate final-code JSONL files using quality proxies.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print(
        f"[done] quality diagnostic selected rows saved to {args.output_jsonl}",
        file=sys.stderr,
    )
    return 0
