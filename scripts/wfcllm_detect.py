#!/usr/bin/env python
"""Run WFCLLM strict final-code-only detection."""
...
from wfcllm.detection.calibration import (  # noqa: E402
    load_calibration_artifact,
    write_calibration_artifact,
)
from wfcllm.detection.config import WFCLLMDetectionConfig  # noqa: E402
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    calibrate_parser = subparsers.add_parser(
        "calibrate",
        help="Build a WFCLLM detector calibration artifact from negative final-code rows.",
    )
    ...
    detect_parser = subparsers.add_parser(
        "detect",
        help="Score final-code rows with a calibrated WFCLLM detector.",
    )
    ...
    evaluate_parser = subparsers.add_parser(
        "evaluate",
        help="Evaluate positive and negative WFCLLM detector detail rows.",
    )
...
def _build_pipeline(args: argparse.Namespace) -> WFCLLMDetectionPipeline:
    ...
    scorer = load_wfcllm_window_scorer(
        config=config,
    ...
    return WFCLLMDetectionPipeline(config=config, scorer=scorer)
...
def _cmd_calibrate(args: argparse.Namespace) -> int:
    ...
    print(f"[完成] WFCLLM calibration artifact saved to {output_path}", file=sys.stderr)
    return 0
...
def _cmd_detect(args: argparse.Namespace) -> int:
    ...
    print(f"[完成] WFCLLM detection details saved to {output_path}", file=sys.stderr)
    return 0
...
def _cmd_evaluate(args: argparse.Namespace) -> int:
    ...
    print(f"[完成] WFCLLM detection report saved to {output_path}", file=sys.stderr)
    return 0
...
def _cmd_split(args: argparse.Namespace) -> int:
    ...
    print(f"[完成] WFCLLM task splits saved to {output_dir}", file=sys.stderr)
    return 0
