...
> 方案详见：[docs/design/基于语句块的语义特征的生成时代码水印方案.md](docs/design/基于语句块的语义特征的生成时代码水印方案.md)
...
`pass@1` 会始终输出；当 `--num-candidates >= 10` 时输出 `pass@10`，否则输出对应候选数的 `pass@k`，并基于每个任务的多候选 `generated_code` 使用本地参考解的 AST 归一化匹配离线计算。
