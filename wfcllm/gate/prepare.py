...
def prepare_gated_experiment(
    *,
    ...
    manifest = _source_manifest_payload(source_catalog)
...
def prepare_gated_source_manifest(
    *,
    source_catalog: Path,
    source_manifest: Path,
) -> dict[str, object]:
    """Prepare only variant-local public provenance for shared family resources."""

    manifest = _source_manifest_payload(source_catalog)
    if source_manifest.exists() or source_manifest.is_symlink():
        raise ValueError("gated experiment preparation refuses to overwrite outputs")
...
def _source_manifest_payload(source_catalog: Path) -> dict[str, object]:
...
__all__ = ["prepare_gated_experiment", "prepare_gated_source_manifest"]
